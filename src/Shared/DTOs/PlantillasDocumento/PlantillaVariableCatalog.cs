...
public static class PlantillaVariableCatalog
{
    ...
    /// <summary>
    /// Variables exclusivas de plantillas de certificados de capacitación.
    /// Solo están disponibles cuando se genera desde el módulo de Capacitaciones.
    /// </summary>
    public static readonly IReadOnlyDictionary<string, string> CertificadoDefinitions = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
    {
        ["{{nombre_capacitacion}}"]     = "Nombre de la capacitación o curso",
        ["{{puntaje}}"]                 = "Puntaje obtenido en el cuestionario (ej: 85%)",
        ["{{duracion_horas}}"]          = "Duración total de la capacitación (en horas)",
        ["{{fecha_emision}}"]           = "Fecha de emisión del certificado (dd/MM/yyyy)",
        ["{{fecha_emision_larga}}"]     = "Fecha de emisión en formato largo (ej: 14 de julio de 2026)",
        ["{{fecha_inicio}}"]            = "Fecha de inicio de la capacitación (dd/MM/yyyy)",
        ["{{fecha_fin}}"]               = "Fecha de finalización de la capacitación (dd/MM/yyyy)",
        ["{{descripcion_capacitacion}}"]= "Descripción o resumen de la capacitación",
    };
}
